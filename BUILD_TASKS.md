...
- [x] **Built 7 Sep 2026 (was: "known gap, not scheduled anywhere").** `identity/OnboardingScreen.kt` — name + barangay + an auto-located, map-adjustable home pin; the first screen on an unregistered device per PRD §9, with SOS as the only way past it. Original note kept for the record: no registration/onboarding screen existed (PRD §9), so `identity/LocalIdentity.kt` is a stand-in — a per-install generated placeholder name/id, clearly not a real collected identity. Real registration should land before this goes near an actual user.
...
- [x] **Background push — FINISHED (kept, 19 Sep, user's call)** — `sync/SupabaseSyncWorker.kt` (WorkManager, 15 min, network-connected). A report made offline uploads once the phone is online, even if the app was closed or the phone rebooted. Unit-tested only; not yet exercised on a real phone with the app killed.
