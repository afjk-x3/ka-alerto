...
**It is not the first screen, and that is the load-bearing decision.** The map is readable by anyone immediately; the form appears the first time somebody tries to **author** — a report, a confirm, a dispute. Blocking a stranger from seeing where the water is in order to collect a name they could type as "x" would be hostile in the exact moment the app exists for, and produces *worse* data: a form in the way of an emergency gets filled with anything. Asking at the point of authorship is where the social cost actually lives. **SOS is never gated**, and the artboard's red "May emergency ka na ngayon?" banner sits on the form itself as the escape hatch.
...
3. **The barangay is not filled from GPS, and the label does not claim it is.** The artboard says "Nakuha sa GPS mo", which needs reverse geocoding, which needs a network — on the one screen most likely to be used without one. It is pre-filled from `DemoArea` and says so. Changing it is free text: shipping a list of San Nicolas's barangays would mean typing real place names nobody here has verified, the same fabrication problem the seats-not-people roster avoided.
