...
| 11a | Family check-in circles (named, shareable join code + QR) | Done on two emulators; QR camera step not proven on hardware |
...
| 12 | SMS fallback | **Not built, on purpose.** Sending SMS is charged, so it stays a placeholder (`SmsReceiver.kt` stub, manifest permissions, the SOS status screen's SMS row reads "Not built yet"). Nothing is sent or decoded by SMS |
...
- **Sync:** only `flood_report`/`confirm`/`dispute`/`official_status`/`flood_withdraw`/`evac_status`/`evac_centre`/`circle_create`/`circle_join` and the redacted `sos*` types go to Supabase (it has no access control); push AND pull have no cursor and no bbox on purpose (carry-forward; a demo-area bbox once silently dropped every real-GPS report); `encodeDefaults = true` is required or PostgREST rejects the batch (`PGRST102`); `SupabaseSyncWorker` repeats the push when the app is closed.
- **Family:** `family_checkin` rides the mesh in the clear — disclosed, not fixable without crypto; `circle_create`/`circle_join` do too, AND are also synced to Supabase (see `sync/SupabaseSync.kt`'s allowlist) — a real household's membership and chosen name now sit in Supabase's access-control-free table permanently, a bigger exposure than the pairwise `circle_invite` model it replaced, accepted for a join code that works from anywhere; "my status" reads only this device's own check-ins (`myLastCheckInMs`).
...
- **Demo chip:** the small "Demo" chip on the map (`map/CameraControls.kt`) stays visible in every build. It is how the demo jumps back to San Nicolas from anywhere; the release APK is the demo build, so do not hide it behind a build flag.
- **Permissions:** the map explains what is missing once (`PermissionPrefs`, `shouldShowPrimer`) and never fires system dialogs by itself; a denied row on registration or the profile screen becomes "Buksan ang settings". SOS is never gated. Camera is not in the map's dialog because only the Family QR scanner uses it.
...
| **The SMS fallback is a placeholder, not a feature** | Decided 21 Sep. Sending SMS costs money per message (and a gateway number), so it was designed but not built. The README, this file and the SOS status row all say so; do not describe SMS as working. Reopening it means choosing a gateway number and a per-message budget first. |
...
Immutable append-only event store → deterministic reducer → displayed state. Every device holds its own store and computes its own map; **the local DB is a replica, not a cache**. Two devices with the same events must display the same status (NFR-4) — this is the invariant that makes an offline phone trustworthy rather than merely stale. Events reach other devices by two transports: Supabase sync when there is internet, and Bluetooth relay via Nearby Connections (low power, BLE only). A third, SMS (bit-packed, 160 chars), is designed only and not built (see the decisions table). Deduplication is by content hash, so re-delivery over multiple transports is harmless. Notifications are evaluated locally on every event insert, so they fire with no push server. Full detail in `docs/03-architecture.md`.
...
- ~~All Filipino copy is unreviewed by a native speaker.~~ **Closed by decision, 23 Sep 2026** — still not reviewed by a native speaker, but the user chose to stop tracking it as an open task rather than pursue the review. The 6 September pass found English strings sitting inside Filipino screens; all were fixed by 21 September (the last two were the filter chip "Impassable" and the detail sheet's "Dispute"). What remains English in Filipino mode is deliberate loanwords (`Camera`, `Settings`, `Barangay hall`, `Responder`), the map's "Demo" chip, SOS wire values and exception text. A search for `tr()` calls whose Filipino and English text are identical is the quick way to catch a new one.
...
- ~~No field measurement of relay range or battery cost.~~ **Closed by decision, 23 Sep 2026.** Both were attempted: range was blocked (no second device to test with, never faked); the battery-cost run was inconclusive (the phone stayed on USB power for the whole window, so Android tracked zero discharge — an unplugged run would be needed for a real number). Named honestly in the pitch rather than implied to be done. Delivery rate was never attempted and stays open.
