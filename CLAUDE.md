...
**Five of the 6 September test-pass findings were fixed on 7 September. 110/110 unit tests and lint pass.** What changed, and what to not undo:

1. **`LocationFetcher` is bounded and has the fallback its doc always promised.** A fresh `getCurrentLocation` attempt is capped at `FRESH_FIX_TIMEOUT_MS` (6 s — short of the 30 s at which the SOS state machine gives up on every channel), then it falls back to `lastLocation` if that fix is under 10 minutes old, then null. The SOS button and the confirm/dispute actions gained the pending state only "Mag-ulat" had; `submitting` used to block a double-tap without changing the label, which reads as broken for the same reason the SOS button did.
2. **Seed fixture annotations moved out of `note`.** `note` is the resident's own words and *is* rendered; commentary now lives in `fixtureNote`, which the loader drops (`ignoreUnknownKeys`). `_meta.fixtureNoteConvention` records the rule so the two do not get merged back. Report 019 needed a real resident line written for it — "Humupa na, nadaanan ko kanina lang."
3. **`SosContext.PEOPLE_OPTIONS` is ASCII now** (`"2-4"`, not `"2–4"`), with `peopleLabel()` putting the en dash back at display time only. `SosWireValuesTest` asserts every stored answer across all five option lists is printable ASCII — this was going to break day 12's GSM 7-bit packing on a field a rescuer reads, and nothing would have caught it until the SMS leg existed.
4. **The pack banner no longer replaces the map header.** It stacks beneath it, so the role badge and mesh line stay reachable — on a fresh install with no network the old behaviour made the role screen unreachable entirely. Copy is Filipino, and a download that cannot start now says "Naghihintay ng koneksyon" plus what still works, instead of sitting at "0%" forever.
5. **`UNREACHABLE` splits on whether any channel is actually live** (`SosChannelRow.anyBroadcasting()`). With one up it still reads "Patuloy ang pag-broadcast"; with none it reads "Walang maabot ngayon. Awtomatikong susubok ulit ang phone mo." Neither is worded as a failure — §6.2's rule stands — but only one of them is a claim the app can support.

**The day-4 reducer finally has its own test file** (`data/ReducerTest.kt`, 20 tests): Rules A-D, the weighting that feeds them, Wilson confidence, buckets, staleness, and the anchor. **Two of its assertions were wrong on the first run and the reducer was right both times** — worth knowing, because both are easy to "fix" in the wrong direction:

- Three residents at S1 against one S3 *does* lower the display. That is Rule B's bar being met, not a Rule A violation. Reading Rule A as absolute and making it so would mean nothing could ever be marked passable again once one person had called it impassable.
- The proximity anchor is the **centroid of the `flood_report`s**, so two "distant" reports out of three drag the anchor toward themselves and become the proximate ones. Correct for a feature that has genuinely moved; it also means a test about distance must use confirms/disputes, which do not move the anchor.

**The last two from that pass were fixed the same day, and both are easy to undo.**

**Storm now darkens the map surface** (`map/StormMapStyle.kt`). It **re-tints the already-loaded style in memory rather than loading a dark style URL** — offline packs are style-scoped, so a second style means a second pack, and a resident who downloaded only the light one would get a blank map the instant they declared Storm, which is the exact failure the offline gate exists to prevent. Re-tinting what is already cached cannot fail offline. Layers prefixed `kaalerto-` are skipped, so **severity markers keep their exact colours** (`SeverityColors.kt`'s rule that a colour meaning "impassable" cannot shift with a display setting) and end up *more* legible against the dark basemap, not less. The basemap's own POI sprites are dimmed to 45% opacity; ours are never reached. Category is guessed from layer id because a vector style's paint is expressions, not colours to darken arithmetically — an unrecognised fill falls back to *land*, deliberately, since mistaking ground for water on a flood map is the error that matters.

**Toggling the mode reloads the style**, which is what makes the tint reversible without stashing original paint values. Two consequences to keep: `MapScreen` now loads the style in a `LaunchedEffect(maplibreMap, stormMode)` rather than in the `AndroidView` factory, and every marker effect keys on a `styleEpoch` counter — a style reload discards every source and layer, *including ours*, so without that the basemap would re-tint and the markers would vanish. The camera is placed once (`cameraPlaced`), or toggling Storm would throw away the user's pan.

**The evacuation control is labelled "Silungan" instead of a bare house glyph.** It sat in the bottom-right corner — the exact position and icon every map uses for "my home" or "recentre" — in an app that has a home-location feature two gestures away on the same screen. Nobody looking for a shelter would have found it and anyone looking for their home radius would have landed there. The screen's own title changed from "Evacuation centre" to "Mga silungan" to match.

**PRD §9's registration screen was built on 7 September** (`identity/OnboardingScreen.kt`, `NameFormat.kt`), from `design/artboards/Onboarding.dc.html`. It had a finished spec and a finished design and **no build day assigned to it** — BUILD_TASKS.md called it "a known gap, not scheduled anywhere" — so `LocalIdentity`'s generated "Residente 89A7" was standing in for a real collected identity, which meant the decision that *a name exists so a false report has a social cost* was inert: nobody in the barangay knows who 89A7 is. 10 new tests; 120/120 and lint pass.

**It is the first screen on an unregistered device**, per PRD §9's literal "required at first run". (It shipped first as an authoring gate — map readable, form on first author — and the user changed that call the same day; the authoring gate in `KaAlertoApp.gated` stays as a second line of defence so the invariant "nothing is ever authored anonymously" survives a routing change.) **SOS is never gated**, and with a hard gate that stops being a courtesy: the artboard's red "May emergency ka na ngayon?" banner is the *only* thing reachable from the form, and somebody installing mid-flood depends on it. Verified on device — tapping it from a fresh unregistered install goes straight to the SOS hold screen.

**The gate resumes what it interrupted.** `Screen.Onboarding(resume: Screen?)` carries the destination, so finishing the form lands on the report screen with the GPS fix already taken rather than back at the map with the 6 s fetch to do again; a gated confirm/dispute reopens the detail sheet it stopped (`focusFeatureRef`). The interrupted action may be somebody standing in rising water — making them find their way back to it is not a neutral cost.

**Four things about it that are easy to undo:**

1. **The full name is stored, and the containment is structural rather than careful.** The user's decision (7 Sep) was to keep "Juan Dela Cruz" locally so the display rule can change later — against a recommendation to store only the short form. So: it lives under two prefs keys read only by `registeredFirstName`/`registeredLastName`, for pre-filling the edit fields, `Identity.authorName` is *always* `displayFormOf` it, and `NameFormatTest` asserts across all three roles that no embedded name carries the surname. Verified on device: the event reads `Juan D.` while prefs hold `Juan Dela Cruz`. **Do not add a second reader without re-running that test.**
2. **The given name and the surname are two fields, and that is not a UI preference.** This shipped first as one field with the initial taken from the second word — right for "Juan Dela Cruz", wrong for "Juan Carlos Santos", which rendered "Juan C.": an initial belonging to the person's own given name. Compound *given* names (Juan Carlos, Maria Cristina, John Paul) and compound *surnames* (Dela Cruz, De Guzman, San Jose) are both ordinary here, and **no rule over a single string can tell them apart**, so it is stated rather than guessed. The surname is optional — plenty of people go by one name, and a form that refuses them is wrong about a real person rather than catching a fake one. A pre-existing single name is split on its first space so an already-registered device is not asked again: best effort by definition, applied only to stored names and never to anything typed since.
3. **GPS gives a pin, not a barangay name.** The artboard's "Nakuha sa GPS mo" implies a place name, which needs reverse geocoding and therefore a network, on the one screen most likely to be used without one. A *coordinate* needs neither, and it is what the app actually consumes — so the home is found automatically on entry, shown with its accuracy, and adjustable on the real map (`Screen.PickHome`, reusing day 3's tap-to-pick). A hand-placed pin drops the accuracy line rather than keeping a GPS figure that no longer describes it. **Registering also seeds day 5's home radius**, which is what makes the "Abiso — baha malapit sa bahay mo" primer on that same screen true from first launch instead of waiting for somebody to discover the map long-press. The barangay *name* beside it is covered separately below.
4. **The barangay follows the pin, until somebody corrects it.** `describePlace` returns a structured `Place`, so the geocoder's `subLocality` — where a Philippine barangay normally lands — pre-fills the field, and moving the pin moves it too; leaving the barangay behind when the pin moves would quietly file reports against the wrong place. Once the person edits it, it stops following: a field that keeps overwriting a correction is worse than one that never filled itself, because they have already told the app it was wrong once. The caption says which of the two happened, since a value read off the location and this build's demo-area default deserve different amounts of trust. Offline the geocoder returns nothing and the previous value stands. Verified on device with the emulator GPS in Manila — the field auto-filled to "Quiapo".
5. **No SMS permission row.** The artboard offers four primers; SMS is build day 12. A setup screen is where people expect every switch to work, so a dead one reads as broken rather than scheduled — §6.4.4's rule applies here as much as on the SOS status screen. Notifications and nearby-devices are both real today and both ship.

**The registration draft lives in `KaAlertoApp`, not in the screen.** Confirming the pin navigates to the map and back, and a name typed before that has to survive the trip — hoisting it is what makes "Ituro sa mapa" safe to offer at all. Pick-mode's banner is caller-aware for the same reason: telling somebody setting their house that they are placing a *report* location is the small wrongness that makes a person distrust the next screen too.

**Editable, with the consequence on screen.** Reachable from the role screen ("PANGALAN MO · Baguhin"). New events carry the new name; already-relayed ones keep what they were sent with, because the log is append-only — the form says exactly that. `authorId` never changes, so the mesh, the role fold and the false-alarm history all stay intact. Existing installs are prompted once and keep their old events under the placeholder; **do not "fix" that by rewriting them** — any copy already on another phone keeps the old name, so the two devices would permanently disagree and break NFR-4.

**The home pin is real GPS, and three things about it were fixed the same day.** The suspicion that prompted this is worth recording, because the evidence looked damning: every screenshot showed the pin at `18.1709, 120.6058`, which is `DemoArea.centre` to four decimals. It was a coincidence of testing — the emulator had that exact fix injected. Proved by injecting Manila instead (`14.59950, 120.98420`); `fetchCurrentLocation` is `FusedLocationProviderClient` and always was.

1. **The adjust-map camera was hardcoded to `DemoArea.centre`.** `MapScreen` moved the camera there on style load regardless of where the device was, so somebody in Manila tapping "Ituro sa mapa" was shown San Nicolas — and **a single tap then silently relocated their home 400 km**, replacing an honest "GPS ±5 m" with "Nakatakda" and no hint anything had moved. Observed exactly that on device. `initialCamera` is a parameter now, and `Screen.PickHome` passes the pin being corrected.
2. **A claimed second bug that was not one.** The pick map appeared to have no location dot, and that was reported as a defect. `MapScreen` computes `showLocation` from the permission state itself — the dot was never missing, it was off-screen because the camera was in the wrong part of the country. One bug, not two; the camera fix resolves both symptoms.
3. **Setting a home now streams for the best fix rather than taking the first one** (`fetchAccurateLocation`). `getCurrentLocation` returns whichever fix arrives first, which on a cold start indoors is the *worst* of the series — commonly ±50-100 m, tightening over the next few seconds. Accepting it would quietly place a home up to a block from the house, and that home then decides for months whether a flood is near enough to wake somebody. It collects up to 15 s, returns early at ≤20 m, and degrades to the best seen, then to the 6 s one-shot, then null. **The two budgets are deliberately different**: an SOS needs *a* position now and refines afterwards; a home is set once. Do not unify them.

**An out-of-area guard, because the limitation was previously invisible.** Every fixture — seeds, evacuation centres, routes, the 8-tile pack — is frozen to one barangay, so a home outside `DemoArea.bounds` gets a blank map and will never have a report near it. The pin is still kept exactly as found (it is not wrong, just unserved); the form now says so in amber, and says SOS still works. Discovering that during a flood instead of at registration is the failure this prevents.

**A second offline pack, built around the resident's own home** (`OfflineMapPack` is region-parameterised now; `HOME_REGION_NAME`, `boundsAround`). This started as "bundle a second pack for outside the demo area", which **cannot exist**: an offline region is a bbox × zoom range, and the Philippines at z10-14 is on the order of 300,000 tiles — several GB, in an APK. A pack around *one person's* home is a few dozen tiles, so that is what gets built, at registration, while they still have a signal — which is exactly what the artboard's "I-download habang may signal pa" always meant.

It is created **only when the pin falls outside `DemoArea.bounds`** — inside it the demo pack already covers them and a second would be wasted bytes. Verified end to end on device with the emulator GPS in Manila: demo pack 8 tiles / 198 KB, home pack **14 tiles / 2.27 MB**, and then, in airplane mode after a force-stop, Quiapo rendering with streets, barangay labels and LRT stations, the blue dot, and the 300 m home radius drawn around it. The size difference is worth remembering — dense urban tiles cost roughly ten times rural ones, so "a few dozen tiles" is not a fixed budget.

`HOME_HALF_EXTENT_M` is 1.5 km each way. Bigger would turn a setup screen into a multi-minute download on a possibly metered connection; smaller stops covering the walk a flood actually changes.

**The home row shows a place name above the coordinates** (`location/PlaceName.kt`). `Geocoder` is a network client on essentially every device, so this returns null offline and the row says so rather than leaving a blank where a name was — **the coordinate stays visible underneath in both cases**, because it is what the app stores and acts on and the name is a courtesy label over it. Registration is the one moment a connection is likely, which is why the name is asked for here and nowhere else. Seen resolving "Quiapo, Manila" on device.

**The out-of-area guard's copy was corrected when the home pack landed.** It used to say there would be no map; there is one now. It says the map of your area downloads on Magsimula, that flood *reports* remain demo-area only, and that SOS still works. A warning that is out of date is worse than no warning, because it is the one thing on the screen somebody has been asked to trust.
