...
**Still open from that pass:** Storm mode leaves the map surface bright (needs a dark style, and the offline pack is style-scoped so that means a second pack or a raster invert — a design call, not a patch), and the evacuation entry is still a house glyph in the corner where maps put "recentre".
