...
**Build day 1 (offline map tiles) is in progress against a live style, not tested on hardware yet.** `OfflineMapPack.kt` and `MapScreen.kt` implement the `OfflineManager` pre-download path against MapLibre's free demo style (`demotiles.maplibre.org`) — not the self-hosted OSM-derived style the architecture doc calls for, and not yet the bundled-MBTiles bulletproof fallback. The code builds clean (lint 0 issues, unit tests pass) but the actual day-1 DoD — airplane mode, force-stop, relaunch, tiles render — is unverified: no physical device has been connected yet (`adb devices` has shown at most an emulator, which does not count for anything requiring real radios).
