...
/**
 * Day 5's Storm Mode, finally applied to the map surface as well as the chrome.
 *
 * Until 7 September the toggle inverted the header, filter bar, legend and action bar,
 * and left the map itself a bright white rectangle filling roughly 60% of the screen —
 * in the one mode built for a dark night and a dying battery.
 *
 ...
 * the moment they declared Storm — which is the exact failure the whole offline gate
 * exists to prevent. Re-tinting what is already cached costs nothing and cannot fail
 * offline.
 *
 * **Our own layers are never touched.** Everything the app draws is prefixed
 * [OURS_PREFIX], and severity colour is constant across Normal/Storm/Survival by
 * decision (`ui/theme/SeverityColors.kt`) — a colour meaning "impassable" cannot shift
 * with a display setting. So the basemap goes dark underneath and the markers stay
 * exactly as bright as they were, which is also what makes them *more* legible here
 * rather than less.
 */
